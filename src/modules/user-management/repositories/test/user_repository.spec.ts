import { MongoClient } from "mongodb";
import { IUserRepository } from "../iuser_repository";
import { MongoDBConnection } from "../../../../utils/mongodb_connection";
import { UserRepositoryMongodb } from "../user_repository_mongodb";
import { ksuidSync } from "../../../../utils/identifier";
import { User } from "../../core/user";

let userRepo: IUserRepository;
let mongoClient: MongoClient;
let testIds: string[] = [];

beforeAll(async () => {
    mongoClient = await MongoDBConnection();
    userRepo = new UserRepositoryMongodb(mongoClient);
    
    const userId = ksuidSync();
    testIds.push(userId);

    await userRepo.insert(new User({
        id: userId,
        userName: "Redi sample",
        accountNumber: "8239283",
        emailAddress: "rediramdan02@gmail.com",
        identityNumber: "02930239"
    }));
});

afterAll(async () => {
    await Promise.all(
        testIds.map(async (id: string) => {
            await userRepo.delete(new User({
                id,
                userName: "User 2",
                accountNumber: "037383434",
                emailAddress: "user2@gmail.com",
                identityNumber: "932483849"
            }));
            return null;
        })
    );

    mongoClient.close();
});

describe("User Repository Spec", () => {
    test("Successfully find all user from database", async () => {
        const users = await userRepo.find();

        expect(users?.length).toBeGreaterThan(0);
    }); 

    test("Successfully find one user from database", async () => {
        const user = await userRepo.findOne(testIds[0] as string);

        expect(user?.id).toEqual(testIds[0]);
        expect(user?.userName).toEqual("Redi sample");
        expect(user?.accountNumber).toEqual("8239283");
        expect(user?.emailAddress).toEqual("rediramdan02@gmail.com");
        expect(user?.identityNumber).toEqual("02930239");
    }); 

    test("Successfully insert user to database", async () => {
        const userId = ksuidSync();
        testIds.push(userId);

        await userRepo.insert(new User({
            id: userId,
            userName: "User 2",
            accountNumber: "037383434",
            emailAddress: "user2@gmail.com",
            identityNumber: "932483849"
        }));

        const user = await userRepo.findOne(userId);
        expect(user?.id).toEqual(userId);
        expect(user?.userName).toEqual("User 2");
        expect(user?.accountNumber).toEqual("037383434");
        expect(user?.emailAddress).toEqual("user2@gmail.com");
        expect(user?.identityNumber).toEqual("932483849");
    });

    test("Successfully update user in database", async () => {
        const userId = testIds[1];
        
        await userRepo.update(new User({
            id: userId,
            userName: "User 2 edit",
            accountNumber: "0373837674",
            emailAddress: "user2+edit@gmail.com",
            identityNumber: "833934044"
        }));

        const user = await userRepo.findOne(userId);

        expect(user?.id).toEqual(userId);
        expect(user?.userName).toEqual("User 2 edit");
        expect(user?.accountNumber).toEqual("0373837674");
        expect(user?.emailAddress).toEqual("user2+edit@gmail.com");
        expect(user?.identityNumber).toEqual("833934044");
    });

    test("Successfully delete user in database", async () => {
        const userId = testIds[1];
        
        await userRepo.delete(new User({
            id: userId,
            userName: "User 2 edit",
            accountNumber: "0373837674",
            emailAddress: "user2+edit@gmail.com",
            identityNumber: "833934044"
        }));

        const user = await userRepo.findOne(userId);

        testIds = testIds.filter((id) => id !== userId);
        
        expect(user).toBeUndefined();
    });

    test("Update user should be return undefined if it fails to update", async () => {
        const userId = ksuidSync();
        const user = await userRepo.update(new User({
            id: userId,
            userName: "User",
            accountNumber: "0373837674",
            emailAddress: "user2+edit@gmail.com",
            identityNumber: "833934044"
        }));

        expect(user).toBeUndefined();
    });

    test("Delete user should be return undefined if it fails to delete", async () => {
        const userId = ksuidSync();
        const user = await userRepo.delete(new User({
            id: userId,
            userName: "User",
            accountNumber: "0373837674",
            emailAddress: "user2+edit@gmail.com",
            identityNumber: "833934044"
        }));

        expect(user).toBeUndefined();
    });
});
